 ...
 */
public class TblImplementedSqlClauseCreator implements SqlClauseCreator {
    ...
    protected boolean isCurrentDBDef(DBDef currentDBDef) {
        return TblDBCurrent.getInstance().isCurrentDBDef(currentDBDef);
    }

    protected boolean isInnerJoinAutoDetect() {
        return TblDBFluteConfig.getInstance().isInnerJoinAutoDetect();
    }

    protected boolean isThatsBadTimingDetect() {
        return TblDBFluteConfig.getInstance().isThatsBadTimingDetect();
    }

    protected boolean isNullOrEmptyQueryAllowed() {
        return TblDBFluteConfig.getInstance().isNullOrEmptyQueryAllowed();
    }

    protected boolean isEmptyStringQueryAllowed() {
        return TblDBFluteConfig.getInstance().isEmptyStringQueryAllowed();
    }

    protected boolean isOverridingQueryAllowed() {
        return TblDBFluteConfig.getInstance().isOverridingQueryAllowed();
    }

    protected boolean isColumnNullObjectAllowed() {
        return TblDBFluteConfig.getInstance().isColumnNullObjectAllowed();
    }

    protected boolean isColumnNullObjectGearedToSpecify() {
        return TblDBFluteConfig.getInstance().isColumnNullObjectGearedToSpecify();
    }

    protected boolean isDatetimePrecisionTruncationOfCondition() {
        return TblDBFluteConfig.getInstance().isDatetimePrecisionTruncationOfCondition();
    }

    protected boolean isDisableSelectIndex() {
        return TblDBFluteConfig.getInstance().isDisableSelectIndex();
    }
}
