 ...
 */
public class TblDBFluteInitializer {

    // ===================================================================================
    //                                                                          Definition
    //                                                                          ==========
    /** The logger instance for this class. (NotNull) */
    private static final Logger _log = LoggerFactory.getLogger(TblDBFluteInitializer.class);
     ...
     */
    public TblDBFluteInitializer(javax.sql.DataSource dataSource) {
        if (dataSource == null) {
    ...
    protected void standBy() {
        if (!TblDBFluteConfig.getInstance().isLocked()) {
            TblDBFluteConfig.getInstance().lock();
        }
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        final TblDBFluteConfig config = TblDBFluteConfig.getInstance();
        ...
        if (needsSpringTransactionalDataSource(dataSourceFqcn)) {
            config.unlock();
            config.setDataSourceHandler(new TblDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }
