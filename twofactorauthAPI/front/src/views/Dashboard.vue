<template>
  <div class="animated fadeIn">
    <b-row>
      <b-col>
        <b-button variant="outline-primary" @click="sideEnabled">サイドメニューのDisabledをEnabled表示に切り替え</b-button>
      </b-col>
    </b-row>
    <b-row>
      <b-col sm="6" lg="3">
...
export default {
  ...
  methods: {
    ...
    sideEnabled: function() {
      nav.items.forEach(item => {
        if (item.name === 'Disabled') {
          item.badge.variant = item.badge.variant === 'secondary' ? 'primary' : 'secondary';
          item.attributes.disabled = !item.attributes.disabled;
        }
      });
    }
  }
